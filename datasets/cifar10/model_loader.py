import os
import torch
import datasets.cifar10.models.vgg as vgg
import datasets.cifar10.models.resnet as resnet
import datasets.cifar10.models.densenet as densenet
...
def load(model_name, model_file=None, data_parallel=False):
    ...
    """
    net = models_dict[model_name]()   # define net as an empty model, with name model_name
    if data_parallel:  # the model is saved in data parallel mode
        net = torch.nn.DataParallel(net)

    if model_file:
        assert os.path.exists(model_file), model_file + " does not exist."
        with open( model_file , 'rb') as f:   # use initial model as the starting point
            stored = torch.load(f , map_location=lambda storage, loc: storage)
